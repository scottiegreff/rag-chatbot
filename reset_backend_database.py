"""
Reset Backend Database with Clean Data
This script clears the database that the backend is actually using (port 5432).
"""
...
# Database connection - using the same settings as the backend
db_params = {
    'host': os.getenv('DB_HOST', 'localhost'),
    'port': os.getenv('DB_PORT', '5432'),  # Backend uses 5432, not 5433
...
def clear_database():
    ...
    print("🗑️  Clearing backend database (port 5432)...")
...
def main():
    ...
    print("🚀 RESETTING BACKEND DATABASE WITH CLEAN DATA")
    print("=" * 60)
    ...
    try:
        ...
        if is_consistent:
            print("\n🎉 SUCCESS: Backend database reset with clean, consistent data!")
            print("   The chatbot should now get consistent totals.")
        else:
