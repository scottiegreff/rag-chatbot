#!/usr/bin/env python3
"""
Comprehensive benchmark test script for Mistral 7B Instruct v0.2 model.
Optimized with longer timeouts for the larger model.
"""
...
def test_general_knowledge():
    ...
    for i, question in enumerate(questions, 1):
        try:
            payload = {
                "message": question,
                "session_id": f"knowledge_test_7b_{i}"
            }
            start_time = time.time()
            response = requests.post(f"{BASE_URL}/api/chat", json=payload, timeout=TIMEOUT)
            end_time = time.time()
...
def test_reasoning_questions():
    ...
    for i, question in enumerate(questions, 1):
        try:
            payload = {
                "message": question,
                "session_id": f"reasoning_test_7b_{i}"
            }
            start_time = time.time()
            response = requests.post(f"{BASE_URL}/api/chat", json=payload, timeout=TIMEOUT)
            end_time = time.time()
...
def print_summary(math_results, prog_results, knowledge_results, reasoning_results):
    ...
    print(f"   • Model: Mistral 7B Instruct v0.2 (4.16GB) vs TinyLlama 1.1B (668MB)")
    print(f"   • Expected: 20-40% improvement in reasoning and accuracy")
    print(f"   • Trade-off: Longer response times for better quality")

def main():
    print("🚀 Starting Comprehensive Benchmark Test - Mistral 7B Instruct v0.2")
    print(f"⏰ Test started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"⏱️  Timeout set to: {TIMEOUT}s (optimized for 7B model)")
    print("=" * 80)
