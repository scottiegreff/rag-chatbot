...
# FCI Chatbot Docker Deployment Script
...
show_status() {
    ...
    docker volume ls | grep fci-chatbot
}
...
backup_postgres() {
    ...
    if docker-compose ps postgres | grep -q "Up"; then
        docker-compose exec -T postgres pg_dump -U postgres fci_chatbot > "$BACKUP_DIR/postgres_backup_$DATE.sql"
...
backup_weaviate() {
    ...
    if docker volume ls | grep -q "fci-chatbot_weaviate_data"; then
        docker run --rm -v fci-chatbot_weaviate_data:/data -v $(pwd)/$BACKUP_DIR:/backup alpine tar czf /backup/weaviate_backup_$DATE.tar.gz -C /data .
...
restore_postgres() {
    ...
    if docker-compose ps postgres | grep -q "Up"; then
        docker-compose exec -T postgres psql -U postgres -d fci_chatbot -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
        docker-compose exec -T postgres psql -U postgres fci_chatbot < "$backup_file"
...
restore_weaviate() {
    ...
    if docker volume ls | grep -q "fci-chatbot_weaviate_data"; then
        docker run --rm -v fci-chatbot_weaviate_data:/data -v $(pwd):/backup alpine tar xzf "/backup/$backup_file" -C /data
...
# Main script logic
main() {
    echo "🚀 FCI Chatbot Docker Deployment"
    echo "================================"
